...
//    let r = await fetch("testfile-manifold.txt");
    ...
    for (let row of arrData ){
            ...
            switch ( row[colIndex]){
                ...
                case "|":
                    if ( arrBeams[colIndex] ){
                        newArrBeams[colIndex] += arrBeams[colIndex];
                        row[colIndex]="|";
                    }
                break;
                ...
                case "^":
                    if ( arrBeams[colIndex]){
                        ...
                        if ( colIndex > 0 && (row[colIndex-1]==="." || row[colIndex-1]==="|")){
                            newArrBeams[colIndex-1]+=arrBeams[colIndex];
                            row[colIndex-1]="|";
                        }
                        if ( colIndex < arrBeams.length-1 && (row[colIndex+1]==="." || row[colIndex+1]==="|")){
                            newArrBeams[colIndex+1]+=arrBeams[colIndex];
                            row[colIndex+1]="|";
                        }
                    }
        ...
        numTimelines = newArrBeams.reduce( (prev,curr) => prev+curr,0);
        arrResults.push("Tree: "+ row.join("")+" Timelines = "+numTimelines.toString());
        arrBeams = newArrBeams;
    }
    numTotals += numTimelines;
